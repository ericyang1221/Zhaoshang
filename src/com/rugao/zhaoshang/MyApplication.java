    ...
    public void setUserBean(final UserBean userBean) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    String url = URLGenerater.makeUrl(
                            Constants.PROJECT_GETTITLE,
                            new String[]{
                                    String.valueOf(userBean.getUserId()),
                                    userBean.getMemo()});
                    JSONArray ja = getHttpRequestHelper()
                            .sendRequestAndReturnJsonArray(url);
                    Log.d(TAG, "PROJECT_GETTITLE: " + ja.toString());
                    Utils.putProjectTitle(MyApplication.this, ja.toString());
                    projectTitle = Utils.convertJA2SA(ja);
                } catch (Exception e) {
                    e.printStackTrace();
                    projectTitle = Utils.convertJAStr2SA(Utils
                            .getProjectTitle(MyApplication.this));
                }
